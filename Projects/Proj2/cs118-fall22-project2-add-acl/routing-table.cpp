...
RoutingTable::lookup(uint32_t ip) const
{
  ...
  RoutingTableEntry* longestPrefix = (RoutingTableEntry*) & (*itor);

  // lookup a proper entry in the routing table using "longest-prefix match" algorithm
  for (; itor != m_entries.end(); itor++) {
    if (((itor->dest & itor->mask) == (itor->mask & ip)) && (longestPrefix->mask < itor->mask)) {
        longestPrefix = (RoutingTableEntry*) & (*itor);   
    }
  }
