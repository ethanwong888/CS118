...
{
  // FILL THIS IN
  // check to see if the parameters match the following
  //c0a80202&ffff0000 ac40030a&ffff0000 0&0 0&0 0&0 1 deny
  //c0a80202&ffff0000 ac40030a&ffff0000 0&0 0&0 1&ff 2 deny
  // if they do match then apply the rules (deny them)
  // else just return an ACLTableEntry object with the given parameters

  int mLength = m_entries.size();
  for (int i = 0, i < mLength; i++) {
    if ((srcIp == i.srcIP) && (dstIp == i.dstIp) && (protocol == i.protocol) and (srcPort == i.srcPort) && (dstPort == i.dstPort)) {
...
//This function is supposed to add an ACL entry to the ACL table. You are required to implement this function based on your own understanding. No detailed explanation will be given.
void
